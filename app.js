const state = {
    ...
    photos: [],
    goalSwipeStates: {},
    eventSwipeStates: {} // NEW: Track event swipe states
};
...
function initializeApp() {
    ...
    // FIX: Asegurar que todas las fechas empiecen en HOY
...
function loadFromStorage() {
    ...
    if (saved) {
        ...
        if (!state.eventSwipeStates) state.eventSwipeStates = {}; // NEW
    }
}
...
function renderCalendar(containerId) {
    ...
    // FIX: Usar la fecha seleccionada como centro, no currentMonth
    const centerDate = new Date(selected);
    
    // FIX: Comenzar 30 días ANTES de la fecha seleccionada
...
// UNIVERSAL SWIPE TO DELETE FUNCTION - Works everywhere!
...
function toggleTimer(timerKey) {
    ...
    const index = parseInt(indexStr);  // ✅ ADD THIS LINE
    
    // ✅ ADD THIS LINE - Actually get the event from state!
...
function addCustomCategory(type) {
    ...
    if (category && category.trim()) {
        if (!state.accounting.customCategories[type].includes(category)) {
            state.accounting.customCategories[type].push(category);
            saveToStorage();
...
function deleteCustomCategory(type) {
    const select = document.getElementById(type + '-category');
    ...
    const index = state.accounting.customCategories[type].indexOf(selectedCategory);
    if (index > -1) {
        if (confirm(`Delete category "${selectedCategory}"?`)) {
            state.accounting.customCategories[type].splice(index, 1);
            saveToStorage();
...
// FIXED: Update only timers, not swipe states
...
// 1. Activar sincronización en tiempo real
...
// 2. Sobreescribir la función de guardar para usar la nube
